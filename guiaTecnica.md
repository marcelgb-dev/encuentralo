...
## 9. Consejos para IAs que trabajen en este proyecto

### 9.1. Patrones comunes a mantener
- **Doble vista:** toda nueva funcionalidad debe implementarse tanto en `.vista-movil` como en `.vista-escritorio`
- **Modales fuera de las vistas:** los modales globales (etiquetas, confirmaciones) deben estar fuera de ambos `<div class="vista-*">`
- **Formularios GET para filtros:** los filtros usan `<form method="get">`, no POST
- **Selector de formulario en escritorio:** usar siempre `aside.filtros-laterales-contenedor form`, nunca `aside form` (bug de logout)

### 9.2. Posibles mejoras / deudas técnicas
- Los modales de etiquetas están duplicados en `resultados.html` y `favoritos.html` (DRY)
- La ordenación podría centralizarse en lugar de tener JS duplicado
- `CategoriaService.obtenerCategorias()` usa `List.contains()` sin `equals()`/`hashCode()` sobreescrito (funciona por `limpiarCategoriasBusqueda()`)
- No hay tests unitarios ni de integración más allá del test de contexto
- El archivo CSS es muy grande y podría modularizarse

### 9.3. Mock Yelp
Con `mock.json=true` en `application.properties`, la app usa datos locales de `src/main/resources/json/yelp_example.json` sin llamar a la API real. Útil para desarrollo sin conexión.

### 9.4. Endpoints de etiquetas
- `POST /etiqueta/crear` — crea etiqueta (params: `nombre`, opcional `resultadoId`)
- `POST /etiqueta/{id}/borrar` — borra etiqueta permanentemente
- `POST /resultado/{resId}/etiqueta/{etqId}/agregar` — asigna etiqueta a resultado
- `POST /resultado/{resId}/etiqueta/{etqId}/quitar` — quita etiqueta del resultado
